...
static void set_signals(void)
{
    ...
    if (sigaction(SIGTERM, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGTERM)");
#ifdef AP_SIG_GRACEFUL_STOP
    if (sigaction(AP_SIG_GRACEFUL_STOP, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,
			    "sigaction("AP_SIG_GRACEFUL_STOP_STRING ")");
#endif
#ifdef SIGINT
    if (sigaction(SIGINT, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGINT)");
#endif
#ifdef SIGXCPU
    sa.sa_handler = SIG_DFL;
    if (sigaction(SIGXCPU, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGXCPU)");
#endif
#ifdef SIGXFSZ
    sa.sa_handler = SIG_DFL;
    if (sigaction(SIGXFSZ, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGXFSZ)");
#endif
#ifdef SIGPIPE
    sa.sa_handler = SIG_IGN;
    if (sigaction(SIGPIPE, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGPIPE)");
#endif
    ...
    if (sigaction(SIGHUP, &sa, NULL) < 0)
	    ap_log_error(APLOG_MARK,NULL,"sigaction(SIGHUP)");
...
static void join_workers(apr_thread_t * listener, apr_thread_t ** threads)
{
    ...
    for (i = 0; i < ap_threads_per_child; i++) {
        if (threads[i]) {       /* if we ever created this thread */
            ...
            if (rv != APR_SUCCESS) {
		    ap_log_error(APLOG_MARK,NULL,"apr_thread_join:unable to join "
				    "worker thread %d",i);
            }
        }
    }
...
static int make_child(int slot)
{
    ...
    if (!pid) {
        ...
        int status = bindprocessor(BINDPROCESS, (int) getpid(),
                                   PROCESSOR_CLASS_ANY);
        if (status != OK)
		ap_log_error(APLOG_MARK,NULL,"processor unbind failed %d ",
				status);
#endif
...
static int worker_pre_init(apr_pool_t * p)
{
    ...
    ap_daemons_to_start = iniparser_getint(d,"mpm_event:StartServers",-1);
...
AP_DECLARE(int) ap_init(const char *inifile,apr_pool_t **pglobal)
{
	apr_initialize();
...
static int set_listener(apr_pool_t *p,dictionary *d)
{
    ...
    int argc = 2;
    ap_set_listener(p,argc,argv);
    if(ap_open_listeners(p) == -1)
	    return -1;
    return 0;
}

AP_DECLARE(int) ap_mpm_run(apr_pool_t * p)
{
    ...
    if (!is_graceful) {
	    ...
	    if(ap_create_scoreboard(p,SB_SHARED)!=OK)
	    {
    ...
    if (shutdown_pending && !is_graceful) {
        ...
        if (!child_fatal) {
		 ap_log_error(APLOG_MARK,NULL,"caught SIGTERM, shutting down");
        }
        return 1;

...
#define MOD_STORAGE_SET_MUTEX_PERMS /* XXX Apache should define something */
#endif

int storage_mutex_init(MCConfigRecord *mc,apr_pool_t *p)
{
    apr_status_t rv;

    if (mc->nMutexMode == STORAGE_MUTEXMODE_NONE) 
        return TRUE;
...
#ifdef MOD_STORAGE_SET_MUTEX_PERMS
...
int storage_mutex_reinit(MCConfigRecord *mc,apr_pool_t *p)
{
    apr_status_t rv;

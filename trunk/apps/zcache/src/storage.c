...
void storage_init(MCConfigRecord *mc,apr_pool_t *p)
{
	/*
	* Warn the user that he should use the storage cache.
	* But we can operate without it, of course.
	*/
	if (mc->nStorageMode == STORAGE_SCMODE_UNSET) {
	/*	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, s,
			"Init: Session Cache is not configured "
			"[hint: LUASessionCache]");*/
		mc->nStorageMode = STORAGE_SCMODE_NONE;
		return;
	}

	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT
		||mc->nStorageMode == STORAGE_SCMODE_SHMCB) 
	{
		if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
			storage_shmht_init(mc,p);
		else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
			storage_shmcb_init(mc,p);
	}
}

void storage_attach(MCConfigRecord *mc,apr_pool_t *p)
{
/*
	* Warn the user that he should use the storage cache.
	* But we can operate without it, of course.
	*/
		...
		||mc->nStorageMode == STORAGE_SCMODE_SHMCB) 
	{
		if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
			...
			storage_shmcb_attach(mc,p);
	}
}
void storage_kill(MCConfigRecord *mc)
{
	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
		storage_shmht_kill(mc);
	else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
		storage_shmcb_kill(mc);
	return;
}

BOOL storage_store(MCConfigRecord *mc,UCHAR *id, int idlen, time_t expiry, void * pdata, int ndata)
{
	BOOL rv = FALSE;

	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
		rv = storage_shmht_store(mc,id, idlen, expiry, pdata, ndata);
	else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
		rv = storage_shmcb_store(mc,id, idlen, expiry, pdata, ndata);
	return rv;
}

void *storage_retrieve(MCConfigRecord *mc,UCHAR *id, int idlen, int* ndata)
{
	void *pdata = NULL;

	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
		pdata = storage_shmht_retrieve(mc,id, idlen,ndata);
	else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
		pdata = storage_shmcb_retrieve(mc,id, idlen,ndata);
	return pdata;
}

void storage_remove(MCConfigRecord *mc,UCHAR *id, int idlen)
{
	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
		storage_shmht_remove(mc,id, idlen);
	else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
		storage_shmcb_remove(mc,id, idlen);
	return;
}

void storage_status(MCConfigRecord *mc,apr_pool_t *p, void (*func)(char *, void *), void *arg)
{
	if (mc->nStorageMode == STORAGE_SCMODE_SHMHT)
		storage_shmht_status(mc,p, func, arg);
	else if (mc->nStorageMode == STORAGE_SCMODE_SHMCB)
		storage_shmcb_status(mc,p, func, arg);
